...
/* select()
   ...
   Since the PF and SC VMs are intended to be single-threaded for
   simplicity, some kind of event processing loop needs to be
   implemented.

   In the following, the arrays are modified in-place to leave only
   the ports with events available.
...
static inline int to_fdset(fd_set *set, port_extract_fd extract, void *ctx,
                           int nb, void **x, int max) {
    FD_ZERO(set); 
    int i; 
    for (i=0; i<nb; i++) {
        int fd = extract(ctx, x[i]);
        // fprintf(stderr, "FD %d\n", fd);
        FD_SET(fd, set);
        if (fd > max) max = fd;
    }
...
static inline void from_fdset(fd_set *set, port_extract_fd extract, void *ctx, 
                              int nb, void **x, void *not) {
    int i; for (i=0; i<nb; i++) if (!FD_ISSET(extract(ctx, x[i]), set)) x[i] = not;
}
#define TO_FDSET(x) max = to_fdset(&x##set, extract, ctx, nb_##x, x, max)
#define FROM_FDSET(x) from_fdset(&x##set, extract, ctx, nb_##x, x, not)
int ports_select(port_extract_fd extract, void *ctx, void *not,
    ...
    long int isec = sec;
    long int usec = 1000000.0 * (sec - ((double)isec));
    struct timeval tv = {isec, usec};

    TO_FDSET(in);
    TO_FDSET(out);
    TO_FDSET(err);
    if (-1 == (rv = select(1 + max, &inset, &outset, &errset, &tv))) return rv;
    FROM_FDSET(in);
    FROM_FDSET(out);
    FROM_FDSET(err);
    return 0;
